...
def classification_region(theta, p, grid, m1, hc=1, alpha=0.5, t_type='hc'):
    dense = np.zeros(grid)
    ...
    for beta in range(0, grid[0]):
        for r in range(0, grid[1]):
            ...
            #    dense[beta, r] = 0
            #else:
            dense[beta, r] = classification_error(theta, grid_x[beta], grid_y[r], p, m1, hc, alpha, t_type)
...
def classification_error(theta, beta, r, p, m1, cs=1, alpha=0.5, t_type='hc'):
    ...
    for i in range(0, m1):
        ...
        error += sum(y_attempt != y_test) / test_size
    m1 = float(m1)
    #print(error)
    #print(m1)
    return error/m1
